...
function renderOnce() {
  return new _es6Promise.Promise(function (resolve) {
    return resolve();
  }).then(getTheme.bind(this)).then(getMarkdownFilePaths.bind(this)).then(parseMarkdownFiles.bind(this)).then(mergeFrontMatters.bind(this)).then(renderToString.bind(this)).then(clearPublicDir.bind(this)).then(renderToPublicDir.bind(this))
...
function getMarkdownFilePaths() {
  ...
  return new _es6Promise.Promise(function (resolve, reject) {
    (0, _glob2['default'])('**/*.{md,markdown,MD,MARKDOWN}', {
      ...
      _this2.markdownFilePaths = files.map(function (file) {
        return _path2['default'].resolve(_this2.cwd, _this2.config.source_dir, file);
      });
      _npmlog2['default'].verbose('getMarkdownFilePaths', JSON.stringify(_this2.markdownFilePaths, null, 2));
      resolve();
    });
  });
}
