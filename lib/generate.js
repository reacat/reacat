...
function renderOnce() {
  ...
  return new _es6Promise.Promise(function (resolve, reject) {
    new _es6Promise.Promise(function (resolve_) {
      return resolve_();
    }).then(getTheme.bind(_this2)).then(getMarkdownFilePaths.bind(_this2)).then(parseMarkdownFiles.bind(_this2)).then(mergeFrontMatters.bind(_this2)).then(renderToString.bind(_this2)).then(clearPublicDir.bind(_this2)).then(renderToPublicDir.bind(_this2))
